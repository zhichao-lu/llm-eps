...
sys.path.append('../../')
from gls_tsp.eval_helper import readTSPLib
from gls_tsp.eval_helper.gls_run import solve_instance_tsplib
...
class Evaluation():
    ...
    def evaluateGLS(self, heuristic: Callable):
        nins = len(self.instances_name)
        inputs = [(x, self.instances_name[x], self.instances_scale[x], self.instances[x], self.coords[x],
                   self.time_limit, self.ite_max, self.perturbation_moves, heuristic) for x in range(nins)]
    ...
    def evaluate(self, heuristic: Callable):
        ...
        temp_file_path = os.path.join(temp_file_path, '.tsp_lib_res.txt')
        try:
            os.remove(temp_file_path)
        except:
            pass
        self.evaluateGLS(heuristic)
    ...
    def compare_results(self):
        ...
        result_file = os.path.join(cur_path, '.tsp_lib_res.txt')
