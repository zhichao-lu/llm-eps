...
parser.add_argument('--max_input_length', type=int, default=1500)
...
def completions():
    ...
    while True:
        ...
        try:
            ...
            print(f'In {__file__}: Cuda Out of Mem. Current Batch Size is {repeat_prompt}')
...
def format_chat_prompt(messages: list, system_prompt: str = '') -> str:
    texts = [f'<s>[INST] <<SYS>>\n{system_prompt}\n<</SYS>>\n\n']
...
def chat_completions():
    ...
    prompt = format_chat_prompt(messages, system_prompt)
    repeat_prompt = content.get('repeat_prompt', 1)
