...
default_model_path_path = '/raid/zhichao/zhangrui/LLMs/unixcoder-base'
# default_model_path_path = '/Users/zhangrui/Desktop/LLMs/unixcoder-base'
...
class Beam(object):
    ...
    def advance(self, wordLk):
        ...
        # 增加随机性
        bestScores, bestScoresId = flatBeamLk.topk(self.size + 3, 0, True, True)
        random_indices = torch.randperm(bestScores.size(0))[:self.size]
        bestScores = bestScores[random_indices]
        bestScoresId = bestScoresId[random_indices]
        #
        # print(bestScores)
        # print(bestScoresId)
        # bestScores =
...
device = f'cuda:{int(args.d[0])}' if torch.cuda.is_available() else 'cpu'
model = UniXcoder(model_name=pretrained_model_path)
...
def completions():
    ...
    if 'params' in content:
        ...
        beam_size = params.get('beam_size', 3)

    while True:
        try:
            # LLM inference
            tokens_ids = model.tokenize([prompt], max_length=512, mode="<decoder-only>")
            source_ids = torch.tensor(tokens_ids).to(device)
            prediction_ids = model.generate(source_ids, decoder_only=True, beam_size=beam_size, max_length=max_new_tokens)
        ...
        return jsonify(
            {'content': [predictions[0][0], predictions[0][1], predictions[0][2]]}
        )
