...
def generate_datasets():
    ...
    weibull_5k_train = {
        'train_' + str(i): {'capacity': bin_capacity, 'num_items': len(training_data[i]), 'items': training_data[i]} for
        i in range(len(training_data))}
    weibull_5k_val = {
        'val_' + str(i): {'capacity': bin_capacity, 'num_items': len(validation_data[i]), 'items': validation_data[i]}
        for i in range(len(validation_data))}
    weibull_5k_test = {
        'test_' + str(i): {'capacity': bin_capacity, 'num_items': len(test_data_5k[i]), 'items': test_data_5k[i]} for i
        in range(len(test_data_5k))}
    weibull_10k_test = {
        'test_' + str(i): {'capacity': bin_capacity, 'num_items': len(test_data_10k[i]), 'items': test_data_10k[i]} for
        i in range(len(test_data_10k))}
    weibull_100k_test = {
        'test_' + str(i): {'capacity': bin_capacity, 'num_items': len(test_data_100k[i]), 'items': test_data_100k[i]}
