...
class ReEvo:
    ...
    def short_term_reflection(self, population: list[dict]) -> tuple[list[list[str]], list[str], list[str]]:
        ...
        responses_lst = multi_chat_completion(messages_lst, 1, self.cfg.model, self.cfg.temperature, self.local_llm,
                                              self.runtime_config_path)
    ...
    def long_term_reflection(self, short_term_reflections: list[str]) -> None:
        ...
        response = chat_completion(1, messages, self.cfg.model, self.cfg.temperature, self.local_llm,
                                   self.runtime_config_path)
    ...
    def crossover(self, short_term_reflection_tuple: tuple[list[list[str]], list[str], list[str]]) -> list[dict]:
        ...
        for response, worse_code, better_code in zip(reflection_responses_lst, worse_code_lst, better_code_lst):
            # reflection = response[0].message.content  # 改了哈
...
if __name__ == "__main__":
    """
python reevo_bin_packing_or.py problem=bin_packing_or +local_llm=True +runtime_config_path=bin_packing_or_run1.json +run=1
    """
    main()
